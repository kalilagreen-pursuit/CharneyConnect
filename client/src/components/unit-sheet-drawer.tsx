...
export function UnitSheetDrawer({
  ...
  useEffect(() => {
    if (isOpen && unit) {
      console.log(
        `[${actionId}] Unit Sheet opened for Unit ${unit.unitNumber}`,
      );
  ...
  const config = unit
    ? statusConfig[unit.status as UnitStatus] || statusConfig.available
  ...
  const handleAddProspect = () => {
    if (!unit) return;
    console.log(
      `[${actionId}] Opening Quick-Add Prospect form for Unit ${unit.unitNumber}`,
    );
  ...
  const handleLogShowing = () => {
    if (!unit) return;
    console.log(
      `[${actionId}] Opening Log Showing form for Unit ${unit.unitNumber}`,
    );
  ...
  const handleHoldUnit = async () => {
    ...
    try {
      await apiRequest("PUT", `/api/units/${unit.id}/status`, {
        status: "on_hold",
      });
      ...
      console.log(
        `[${actionId}] Unit ${unit.unitNumber} status updated to on_hold`,
      );
    } catch (error) {
  ...
  const getNextBestAction = () => {
    ...
    console.log(
      `[${actionId}] Computing Next Best Action for Unit ${unit.unitNumber}`,
    );
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span
                      ...
                      data-testid="text-unit-building"
                    >
            ...
            <div className="space-y-6 mt-6">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <Card>
                    <CardContent className="pt-6 text-center space-y-2">
                      ...
                      <div
                        className="font-bold"
                        data-testid="text-unit-bedrooms"
                      >
                        {unit.bedrooms}
                      </div>
                  ...
                  <Card>
                    <CardContent className="pt-6 text-center space-y-2">
                      ...
                      <div
                        className="font-bold"
                        data-testid="text-unit-bathrooms"
                      >
                        {unit.bathrooms}
                      </div>
                      <div className="text-xs text-muted-foreground">
                      ...
                      <CardContent className="pt-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div
                              className="font-bold"
                              data-testid="text-floorplan-name"
                            >
              ...
              <div className="space-y-3">
                ...
                <Button
                  variant="outline"
                  size="lg"
                  className="w-full gap-2 min-h-11 border-primary text-primary hover:bg-primary hover:text-primary-foreground"
                  onClick={() => {
                    onClose();
                    setTimeout(() => {
                      // Pass activeVisitId in the URL if available
                      const visitParam = activeVisitId
                        ? `&visitId=${activeVisitId}`
                        : "";
                      window.location.href = `/agent/viewer?mode=3d&unit=${unit.id}&projectId=${unit.project?.id || ""}${visitParam}`;
                    }, 100);
                  }}
                  data-testid="button-view-3d-model"
                >
